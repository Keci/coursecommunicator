...
$('ul', this.container).append('<li><a href="#" style="padding:0;"><label style="margin:0;padding: 3px 20px 3px 20px;width:100%;height:100%;cursor:pointer;font-size:1em;"><input style="margin-bottom:5px;" type="checkbox" value="' + $(element).val() + '" /> ' + $(element).text() + '</label</a></li>');
...
if (selected) {
checkbox.parents('li').addClass('active taglink');
}
}, this));
...
if (checked) {
$(event.target).parents('li').addClass('active taglink');
}
else {
$(event.target).parents('li').removeClass('active taglink');
}
